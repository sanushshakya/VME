...
const EventDetail = () => {
  ...
  const generateTieSheet = async () => {
    try {
      const response = await axios.get(
        `${config.apiBaseUrl}/api/events/read_by_participant/${evtId}`
      );
    ...
    const fetchData = async () => {
      try {
        const resEvt = await axios.get(
          `${config.apiBaseUrl}/api/participants/read_by_event/${evtId}`
        );
        setPar(resEvt.data);
        ...
        const resEvent = await axios.get(
          `${config.apiBaseUrl}/api/events/read_by_id/${evtId}`
        );
  ...
  if (user.role_name == "user") {
    return <div>You're not authorized as Admin.</div>;
  }
