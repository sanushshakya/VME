...
const CreateEvent = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try{
            const resEvt = await axios.post('http://localhost:8000/api/events/create', formData);
                ...
                <div className="bottom">
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="credentials">
                            <span>
                                <input {...register('evt_name')} type="text" placeholder='Event Title'/>
                                <input {...register('date')} type="text" placeholder='Date(YYYY-MM-DD)'/>
                            </span>
                            {errors.evt_name && (
                                <span className="error-message">{errors.evt_name.message}</span>
                            )}
                            {errors.date && (
                            ...
                            <span>
                                ...
                                <input {...register('location')} type="text" placeholder='Venue'/>
                            </span>
                            {errors.location && (
                                <span className="error-message">{errors.location.message}</span>
                            )}
                            <span>
                                <input {...register('deadline')} type="text" placeholder='Deadline(YYYY-MM-DD)'/>
                            </span>
                            {errors.deadline && (
                                <span className="error-message">{errors.deadline.message}</span>
                            )}
                            <span>
                                <input {...register('sub_title')} type="text" placeholder='Event SubTitle'/>
                            </span>
                            {errors.sub_title && (
                                <span className="error-message">{errors.sub_title.message}</span>
                            )}
                            <span>
                                <input {...register('description')} type="text" placeholder='Event Description'/>
                            </span>
                            {errors.description && (
