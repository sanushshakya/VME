...
const Profile = () => {
  ...
  const handleTeamDelete = async(e) => {
    e.preventDefault();
    try {
      const response = await axios.delete(`http://localhost:8000/api/user/delete/${user._id}/${e.target.team_member_remove.value}` )
      setDel(false);
    } catch (error) {
        console.error(error.response?.data || error);
    }
    ...
    <div className='profile'>
      <div className="container">
        <div className="top">
          ...
          <span className='p'>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</span>
          <div className="bio">
            <div className="left">
              {!profile && (
                <>
            ...
            <div className="right">
              ...
              <span>
                {user.role_name!="Admin" && (
                  <>
                    <Link to="/myevents" className='link'><a>My Events</a></Link>
                    <button onClick={handleTeamOn}><a>My Team</a></button>
                  </>
                )}
              </span>
            </div>
        ...
        {teams && (
          <div className="team">
            <span>
              ...
              <button onClick={handleUpdate}><a>Add Members</a></button>
              <button onClick={handleDelete}><a>Remove Members</a></button>
            </span>
            {!update && (
              <table>
                  <tr>
            ...
            {update && (
              <form onSubmit={handleTeamUpdate}>
                <span>
                  <input name="team_member" type='text' placeholder='New Member' />
                </span>
            ...
            {del && (
              <form onSubmit={handleTeamDelete}>
                <span>
                  <input name="team_member_remove" type='text' placeholder='Remove Member' />
                </span>
          ...
          <div className="left">
            ...
            {editing && (
              <form onSubmit={handleSaveClick}>
                <span>
                  <input name = "username" type="text" placeholder='Update your name'/>
                </span>
                <span>
                 <lable>{user.email}</lable>
                </span>
                <span>
                  <input name = "password" type="password" placeholder='Update your password'/>
                </span>
                <span>
